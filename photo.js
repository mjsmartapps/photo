...
async function loadStudioProfile() {
    ...
    try {
        ...
        const profileSnap = await getDoc(doc(db, 'studiousers', uid, 'profile', 'info'));
        ...
        const snap = await getDoc(doc(db, 'studiousers', uid, 'settings', 'profile'));
        if (snap.exists()) {
...
async function updateStat(id, type) {
    ...
    try {
        const clientRef = doc(db, 'studiousers', uid, 'clients', id);
...
async function initializeSystem() {
    try {
        const clientSnap = await getDoc(doc(db, 'studiousers', uid, 'clients', eventId));
...
window.loadFullGallery = async () => {
    ...
    try {
        const mediaCol = collection(db, 'studiousers', uid, 'clients', eventId, 'media');
...
window.submitDeletionRequest = async () => {
    ...
    try {
        ...
        const clientRef = doc(db, 'studiousers', uid, 'clients', eventId);
