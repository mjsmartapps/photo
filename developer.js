...
window.toggleCreationLock = (checkbox) => {
    const isLocked = checkbox.checked;
    update(ref(db), { 'settings/creationLocked': isLocked })
        .then(() => {
...
window.saveProfile = async () => {
    ...
    try {
        await set(ref(db, 'settings/profile'), profileData);
    ...
    Object.entries(data).forEach(([key, client]) => {
        ...
        if (!groupedData[monthKey]) {
            groupedData[monthKey] = {
                ...
                isLocked: false,
                status: 'unpaid',
                meta: null // We will find the correct meta in loop below
            };
        }
        ...
        // Scan for payment status and meta across ALL clients in the month
        // (If any client has meta/paid status, the month is likely paid/partial)
...
function renderGrid(keys, gPaid, gPending) {
    ...
    keys.forEach(monthKey => {
        ...
        // If Paid but has balance > 0, it's Partial
        if(data.isPaid) {
            if(data.displayBalance > 0.01) { // Floating point tolerance
        ...
        // Dynamic Cost Label
        // If Partially Paid, show "Balance Due" in Red. 
        // If Fully Paid, show "Settled" in Green.
        // If Unpaid, show "Est. Cost" in Warning.
        ...
        if (data.isPaid) {
            if (data.displayBalance > 0.01) {
                ...
                costValueDisplay = 0; // Or display 0.00
            }
        }
...
window.performLockUpdate = () => {
    if(!lockTarget.clientIds.length) return;
    const updates = {};
    lockTarget.clientIds.forEach(id => {
        updates[`clients/${id}/isLocked`] = lockTarget.isLocked;
    });
...
window.performBatchUpdate = () => {
    ...
    batchTarget.clientIds.forEach(id => {
        updates[`clients/${id}/paymentStatus`] = batchTarget.status;
        
        if(batchTarget.status === 'paid') {
            // Store batch payment data
            updates[`clients/${id}/paymentMeta`] = {
                lastPaidDate: timestamp,
                batchPaidAmount: paidAmountStr, 
                batchBalanceDue: balanceStr
            };
        } else {
            updates[`clients/${id}/paymentMeta`] = null;
        }
    });
