...
window.renderUserSelectModal = async () => {
    ...
    try {
        ...
        try {
            const usersSnap = await getDocs(collection(db, 'studiousers'));
        ...
        try {
            ...
            clientsSnap.forEach(doc => {
                ...
                if (parts.length >= 3 && parts[0] === 'studiousers') {
        ...
        const profilePromises = Array.from(uniqueUsers).map(async (uid) => {
            try {
                const snap = await getDoc(doc(db, 'studiousers', uid, 'profile', 'info'));
...
function initData() {
    ...
    unsubClients = onSnapshot(collection(db, 'studiousers', targetUserId, 'clients'), (snapshot) => {
...
function initGlobalSettings() {
    ...
    unsubSettings = onSnapshot(doc(db, 'studiousers', targetUserId, 'settings', 'config'), (snapshot) => {
...
window.toggleCreationLock = async (checkbox) => {
    ...
    try {
        await setDoc(doc(db, 'studiousers', targetUserId, 'settings', 'config'), { creationLocked: isLocked }, { merge: true });
...
window.openProfileSettings = async () => {
    ...
    try {
        ...
        const snap = await getDoc(doc(db, 'studiousers', targetUserId, 'settings', 'profile'));
        if (snap.exists()) {
...
window.saveProfile = async () => {
    ...
    try {
        ...
        await setDoc(doc(db, 'studiousers', targetUserId, 'settings', 'profile'), profileData);
...
window.performLockUpdate = async () => {
    ...
    try {
        ...
        lockTarget.clientIds.forEach(id => {
            const docRef = doc(db, 'studiousers', targetUserId, 'clients', id);
